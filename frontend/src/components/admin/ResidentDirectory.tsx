...
type SafehouseOption = { id: string; name: string; capacityGirls: string | null };
...
function CreatePanel({ draft, safehouses, onChange, onSave, onCancel, creating }: CreatePanelProps) {
  return (
            ...
            <div className="detail-grid">
              {fields.map((f) => {
                ...
                if (f.key === 'safehouseId') {
                  const inList = safehouses.some((s) => s.id === value);
                  return (
                    <div key={f.key}>
...
function EditPanel({ draft, safehouses, onChange, onSave, onCancel, saving }: EditPanelProps) {
  return (
    <div className="detail-panel">
      {SECTIONS.map((section) => (
        <div className="detail-section" key={section.title}>
                ...
                return (
                  <div key={f.key}>
                    ...
                    >
                      ...
                      {value && !inList && (
                        <option value={value}>
                          {value} (current — not in directory)
                        </option>
                      )}
                      {safehouses.map((s) => (
      ...
      {!loading && !error && (
        <div className="data-table">
          ...
          <PaginationControls
            ...
            label={`Showing ${displayed.length === 0 ? 0 : (safePage - 1) * pageSize + 1}–${Math.min(safePage * pageSize, displayed.length)} of ${displayed.length} residents`}
          />
        </div>
      )}
    </div>
